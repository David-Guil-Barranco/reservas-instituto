...
const TABS = [
  { id: 'carritos',  label: 'Carritos / Chromebooks', resources: ['chromebooks', 'carro-grande', 'carro-pequeno'] },
...
function renderHeader() {
  ...
  lbl.textContent = m1 === m2
    ? `${currentWeekStart.getDate()} - ${dEnd.getDate()} de ${m1}`
    : `${currentWeekStart.getDate()} de ${m1} - ${dEnd.getDate()} de ${m2}`;
...
function renderBody() {
  ...
  SLOTS.forEach((slot, idx) => {
    ...
    // Celda del tramo horario
    const tdTime = document.createElement('td');
    tdTime.className = 'time-cell';
    tdTime.innerHTML = `<strong>${slot.id}ª</strong><br><span class="time-range">${slot.start}<br>${slot.end}</span>`;
    ...
    for (let i = 0; i < 5; i++) {
      ...
      currentTab.resources.forEach(resId => {
        ...
        if (resv) {
          if (resv.is_block) {
            btn.className = 'slot-btn slot-block';
            btn.innerHTML = isMulti
              ? `<span class="slot-resource">${escHtml(label)}</span><span class="slot-teacher">🔒 ${escHtml(resv.teacher_name)}</span><span class="slot-group">${escHtml(resv.group_name)}</span>`
              : `<span class="slot-teacher">🔒 ${escHtml(resv.teacher_name)}</span><span class="slot-group">${escHtml(resv.group_name)}</span>`;
          } else {
          ...
          btn.className = 'slot-btn slot-free';
          btn.innerHTML = isMulti
            ? `<span class="slot-resource">${escHtml(label)}</span><span class="slot-free-text">+ Reservar</span>`
            : `<span class="slot-free-text">+ Reservar</span>`;
          btn.addEventListener('click', () => openReserveModal(dStr, slot, resId));
        }
...
function renderNavButtons() {
  document.getElementById('btn-prev').onclick = async () => { currentWeekStart.setDate(currentWeekStart.getDate() - 7); await init(); };
  document.getElementById('btn-next').onclick = async () => { currentWeekStart.setDate(currentWeekStart.getDate() + 7); await init(); };
}
...
function hideModal() { modalEl.classList.add('hidden'); modalBody.innerHTML = ''; pending = null; }
...
function openReserveModal(date, slot, resourceId) {
  ...
  showModal(`
    ...
    ${IS_ADMIN ? `
      <label style="margin-top:12px;display:flex;align-items:center;gap:8px;cursor:pointer;background:#fef2f2;padding:10px;border-radius:6px;border:1px solid #fecaca">
        <input type="checkbox" id="m-is-block">
        <strong style="color:#b91c1c">🔒 Bloquear para todo el curso</strong>
      </label>
    ` : `
  ...
  if (IS_ADMIN) {
    document.getElementById('m-is-block').onchange = (e) => {
      if (e.target.checked) document.getElementById('m-group').value = 'BLOQUEO FIJO';
      else document.getElementById('m-group').value = '';
    };
  }
}
...
async function submitReserve() {
  ...
  showErr('m-err', '');
  if (!name || !group || (!pin && !is_block)) return showErr('m-err', 'Rellena todos los campos.');
  const btn = document.getElementById('m-ok');
  btn.disabled = true; btn.textContent = 'Reservando...';
  const payload = { date: pending.date, slot: pending.slotId, resource_id: pending.resourceId, teacher_name: name, group_name: group, pin, is_block, admin_password: IS_ADMIN ? sessionStorage.getItem('adminPw') : undefined };
...
function openCancelModal(resv) {
  ...
  showModal(`
    ...
    <label style="margin-top:15px">Introduce tu PIN para cancelar
...
function setupAdminIndicator() {
  ...
  if (IS_ADMIN) {
    ...
    `;
    document.getElementById('btn-logout').onclick = () => { sessionStorage.removeItem('adminPw'); window.location.reload(); };
  } else {
...
function openAdminLogin() {
  ...
  document.getElementById('a-ok').onclick = async () => {
    ...
    if (res.error) showErr('a-err', res.error);
    else { sessionStorage.setItem('adminPw', pw); window.location.reload(); }
  };
}
...
function openAdminCancelModal(resv) {
  ...
  showModal(`
    ...
    <p class="modal-sub">${DAY_LONG[d.getDay()]} ${d.getDate()} | Tramo ${resv.slot}</p>
    <p>¿Seguro que quieres borrar ${resv.is_block ? 'el bloqueo fijo' : 'la reserva'} de <strong>${escHtml(resv.teacher_name)}</strong>?</p>
    ${resv.is_block ? '<p style="font-size:0.85rem;color:#b91c1c;margin-top:8px">⚠️ Borrará la franja bloqueada para <strong>todo el curso</strong>.</p>' : ''}
  ...
  document.getElementById('ac-ok').onclick = async () => {
    ...
    btn.disabled = true; btn.textContent = 'Borrando...';
    const res = await apiFetch('DELETE', `/api/admin/reservations/${pending.id}`, { password: sessionStorage.getItem('adminPw') });
    if (res.error) { btn.disabled = false; btn.textContent = 'Borrar definitivamente'; showErr('ac-err', res.error); }
    else { hideModal(); await loadReservations(); render(); }
  };
}
